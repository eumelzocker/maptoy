...
import {
  generateErrorTile,
  generateTransparentTile,
} from "../tiles/errorTile.js";
    ...
    async (request, reply) => {
      try {
        ...
        if (
          ...
          if (request.query.missing === "transparent") {
            const body = await generateTransparentTile(
              service.get(request.params.id).tileSize,
            );
            return reply
              .code(200)
              .type("image/png")
